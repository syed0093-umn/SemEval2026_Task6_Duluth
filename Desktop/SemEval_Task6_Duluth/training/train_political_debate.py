"""
Political DEBATE (DeBERTa) for 3-class Clarity Classification

Model: mlburnham/Political_DEBATE_base_v1.0
- DeBERTa-base fine-tuned on political text data
- Domain-specific pre-training matches our political interview dataset
- Base-sized model (~184M params)
...
print("=" * 80)
print("Political DEBATE for 3-Class Clarity Classification")
print("Model: mlburnham/Political_DEBATE_base_v1.0")
print("=" * 80)
...
parser.add_argument('--output_dir', type=str, default='./political_debate_model',
                    help='Output directory for model')
parser.add_argument('--log_file', type=str, default='./logs/train_political_debate.json',
...
# LLRD - adapted for Political DEBATE model architecture
def get_optimizer_grouped_parameters(model, model_base, learning_rate, weight_decay, llrd_alpha):
    """
    Adaptive LLRD that detects model architecture.
    Political_DEBATE uses DeBERTa architecture internally.
    """
    ...
    # Detect the base model attribute name
    # Political_DEBATE may use 'deberta' or another name
    if hasattr(model, 'deberta'):
        ...
        # Fallback: use standard parameter grouping without LLRD
        print("  WARNING: Could not detect base model attribute, using flat LR")
        return [
    ...
    classifier_params = []
    for name in ['classifier', 'cls', 'output']:
        if hasattr(model, name):
            classifier_params = list(getattr(model, name).named_parameters())
            break
    ...
    if hasattr(base_model, 'pooler') and base_model.pooler is not None:
        optimizer_grouped_parameters.append({
            "params": [p for n, p in base_model.pooler.named_parameters()
                       if not any(nd in n for nd in no_decay)],
            "weight_decay": weight_decay,
            "lr": learning_rate
        })
...
model = AutoModelForSequenceClassification.from_pretrained(
    ...
    ignore_mismatched_sizes=True  # Reinitialize classifier head for 3 classes
)
model.to(device)
...
training_log['architecture'] = model.config.model_type
...
optimizer_grouped_parameters = get_optimizer_grouped_parameters(
    model, model_name, args.learning_rate, args.weight_decay, args.llrd_alpha
)
...
print(f"\n{'=' * 80}")
print("Political DEBATE RESULTS")
print(f"{'=' * 80}")
...
zip_name = 'submission_political_debate.zip'
...
print(f"  Political DEBATE base (this):    F1 = {test_f1:.4f}")
